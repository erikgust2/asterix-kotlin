...
internal fun Cat062CodecSupport.readMode5DataReports(buffer: ByteBuffer): Mode5DataReports =
    decodeCat062Item("I062/110", "mode5DataReports") {
        val indicator = decodeCat062Item("I062/110", "mode5DataReports.indicator") { readCompoundIndicator(buffer) }
        ...
        if (isCompoundSubfieldPresent(indicator, 1)) {
            val octet = decodeCat062Item("I062/110", "mode5DataReports.summary") { buffer.get().toUnsignedInt() }
            summary =
                Mode5Summary(
                    m5 = (octet and 0x80) != 0,
                    id = (octet and 0x40) != 0,
        ...
        if (isCompoundSubfieldPresent(indicator, 2)) {
            val raw = decodeCat062Item("I062/110", "mode5DataReports.pinNationalOriginMission") { buffer.int }
        ...
        if (isCompoundSubfieldPresent(indicator, 3)) {
            positionWgs84 = decodeCat062Item("I062/110", "mode5DataReports.positionWgs84") { readWgs84Position(buffer) }
        }
        if (isCompoundSubfieldPresent(indicator, 4)) {
            geometricAltitudeFeet =
                decodeCat062Item("I062/110", "mode5DataReports.geometricAltitudeFeet") { buffer.short.toDouble() * 25.0 }
        }
        if (isCompoundSubfieldPresent(indicator, 5)) {
            extendedMode1Code =
                decodeCat062Item("I062/110", "mode5DataReports.extendedMode1Code") {
                    ExtendedMode1Code(buffer.short.toUnsignedInt() and 0x0FFF)
                }
        }
        if (isCompoundSubfieldPresent(indicator, 6)) {
            timeOffsetSeconds = decodeCat062Item("I062/110", "mode5DataReports.timeOffsetSeconds") { buffer.get().toDouble() / 128.0 }
        }
        if (isCompoundSubfieldPresent(indicator, 7)) {
            val octet = decodeCat062Item("I062/110", "mode5DataReports.xPulsePresence") { buffer.get().toUnsignedInt() }
            xPulsePresence =
                Mode5XPulsePresence(
                    x5 = (octet and 0x10) != 0,
                    xc = (octet and 0x08) != 0,
