...
internal fun Cat062CodecSupport.readMeasuredInformation(buffer: ByteBuffer): MeasuredInformation =
    decodeCat062Item("I062/340", "measuredInformation") {
        val indicator = decodeCat062Item("I062/340", "measuredInformation.indicator") { readCompoundIndicator(buffer) }
        ...
        if (isCompoundSubfieldPresent(indicator, 1)) {
            sensorIdentification =
                decodeCat062Item("I062/340", "measuredInformation.sensorIdentification") { readDataSourceIdentifier(buffer) }
        }
        if (isCompoundSubfieldPresent(indicator, 2)) {
            position =
                decodeCat062Item("I062/340", "measuredInformation.position") {
                    PolarPosition(
        ...
        if (isCompoundSubfieldPresent(indicator, 3)) {
            heightFeet = decodeCat062Item("I062/340", "measuredInformation.heightFeet") { buffer.short.toDouble() * 25.0 }
        }
        if (isCompoundSubfieldPresent(indicator, 4)) {
            val raw = decodeCat062Item("I062/340", "measuredInformation.lastMeasuredModeCCode") { buffer.short.toUnsignedInt() }
        ...
        if (isCompoundSubfieldPresent(indicator, 5)) {
            val raw = decodeCat062Item("I062/340", "measuredInformation.lastMeasuredMode3aCode") { buffer.short.toUnsignedInt() }
        ...
        if (isCompoundSubfieldPresent(indicator, 6)) {
            val octet = decodeCat062Item("I062/340", "measuredInformation.reportType") { buffer.get().toUnsignedInt() }
            reportType =
                ReportType(
...
internal fun Cat062CodecSupport.writeMeasuredInformation(
    ...
    encodeCat062Item("I062/340") {
        ...
        value.heightFeet?.let {
            buffer.putSignedShort(
                quantize(it, 25.0, "measuredInformation.heightFeet"),
                "measuredInformation.heightFeet",
            )
        }
